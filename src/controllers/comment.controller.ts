...
export class CommentaireController {
  ...
  static async getCommentByPost(req: Request, res: Response): Promise<void> {
    ...
    try {
      const comments: Commentaire[] =
        await CommentaireService.getCommentByPost(postId)
  ...
  static async createReplyToComment(
    ...
    if (!content || !userId || !parentId) {
      res
        .status(400)
        .json({ error: 'Content, parentId and userId are required fields' })
      return
    }

    try {
      const reply = await CommentaireService.createReplyToComment(
        parseInt(commentId),
        content,
        parseInt(userId)
      )
  ...
  static async createComment(req: Request, res: Response): Promise<void> {
    ...
    try {
      const newComment = await CommentaireService.createComment(
        new Commentaire(
          0,
          parseInt(userId),
          parseInt(postId),
